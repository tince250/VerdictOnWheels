...
(deffunction parse-ntriples (?address ?projectname)
	...
	(if (and 
	   ...
	   then
		...
		(printout mkbat "@if exist " ?projectname ".n3 goto :eof" crlf)
		(printout mkbat "@if exist " ?projectname ".nt copy /Y " ?projectname ".nt " ?projectname ".n3 >nul" crlf)
		(printout mkbat "@if exist " ?projectname ".n3 goto :eof" crlf)
		(printout mkbat "@echo WARNING: Missing N-Triples converter; expected " ?projectname ".n3 or " ?projectname ".nt" crlf)
		(close mkbat)
